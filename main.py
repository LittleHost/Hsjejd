...
# ========== КЛАВИАТУРЫ (ПРЕМИУМ ЭМОДЗИ ТОЛЬКО В КНОПКАХ) ==========
def main_kb():
    return ReplyKeyboardMarkup(
        keyboard=[[
            KeyboardButton(text="🎮 Игры в боте", icon_custom_emoji_id=PREMIUM_IDS["game"]),
            KeyboardButton(text="👤 Профиль", icon_custom_emoji_id=PREMIUM_IDS["profile"])
        ]],
...
def games_kb():
    ...
    b.row(InlineKeyboardButton(text="💬 Наш Чат", url=CHAT_LINK, icon_custom_emoji_id=PREMIUM_IDS["chat"]))
    b.row(InlineKeyboardButton(text="🔙 Назад", callback_data="back_main", icon_custom_emoji_id=PREMIUM_IDS["back"]))
    return b.as_markup()

def profile_kb():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="💳 Пополнить", callback_data="deposit", icon_custom_emoji_id=PREMIUM_IDS["deposit"]),
         InlineKeyboardButton(text="💰 Вывести", callback_data="withdraw", icon_custom_emoji_id=PREMIUM_IDS["withdraw"])],
        [InlineKeyboardButton(text="🎁 Промокоды", callback_data="promo", icon_custom_emoji_id=PREMIUM_IDS["gift"]),
         InlineKeyboardButton(text="👥 Реф Программа", callback_data="ref", icon_custom_emoji_id=PREMIUM_IDS["ref"])],
        [InlineKeyboardButton(text="📤 Перевод", callback_data="transfer", icon_custom_emoji_id=PREMIUM_IDS["send"]),
         InlineKeyboardButton(text="🔙 Назад", callback_data="back_main", icon_custom_emoji_id=PREMIUM_IDS["back"])]
    ])

def deposit_kb():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="💎 CryptoBot", callback_data="dep_crypto", icon_custom_emoji_id=PREMIUM_IDS["crypto"])],
        [InlineKeyboardButton(text="⭐️ Telegram Stars", callback_data="dep_stars", icon_custom_emoji_id=PREMIUM_IDS["star"])],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="back_profile", icon_custom_emoji_id=PREMIUM_IDS["back"])]
    ])

def stars_amount_kb():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⭐️ 50 Stars ($0.55)", callback_data="stars_50", icon_custom_emoji_id=PREMIUM_IDS["star"])],
        [InlineKeyboardButton(text="⭐️ 100 Stars ($1.10)", callback_data="stars_100", icon_custom_emoji_id=PREMIUM_IDS["star"])],
        [InlineKeyboardButton(text="⭐️ 250 Stars ($2.75)", callback_data="stars_250", icon_custom_emoji_id=PREMIUM_IDS["star"])],
        [InlineKeyboardButton(text="⭐️ 500 Stars ($5.50)", callback_data="stars_500", icon_custom_emoji_id=PREMIUM_IDS["star"])],
        [InlineKeyboardButton(text="⭐️ 1000 Stars ($11.00)", callback_data="stars_1000", icon_custom_emoji_id=PREMIUM_IDS["star"])],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="back_deposit", icon_custom_emoji_id=PREMIUM_IDS["back"])]
    ])
...
def cancel_kb(callback_data: str = "cancel_action"):
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="❌ Отмена", callback_data=callback_data, icon_custom_emoji_id=PREMIUM_IDS["cross"])]
    ])
...
<i>От 💵100 в день — реально</i>"""
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="📢 Подписаться", url="https://t.me/SecondNewsRU", icon_custom_emoji_id=PREMIUM_IDS["chat"])],
            [InlineKeyboardButton(text="✅ Проверить подписку", callback_data="check_sub", icon_custom_emoji_id=PREMIUM_IDS["check"])]
        ])
        await msg.answer(
...
async def profile_menu(msg: Message):
    ...
    await msg.answer(f"👤 Ваш профиль\n\n💰 Баланс: 💵{u['balance']:.2f}\n📅 Дней в боте: {days}", reply_markup=profile_kb())
...
async def back_profile(c: CallbackQuery):
    ...
    days = (datetime.now() - u["reg"]).days
    await c.message.answer(f"👤 Ваш профиль\n\n💰 Баланс: 💵{u['balance']:.2f}\n📅 Дней в боте: {days}", reply_markup=profile_kb())
    await c.answer()
...
async def play_tower(msg: Message, state: FSMContext, level: int, mult: float, bet: float):
    ...
    b.row(InlineKeyboardButton(text="❌ Отмена", callback_data="cancel_tower", icon_custom_emoji_id=PREMIUM_IDS["cross"]))
                ...
                if result.get("ok"):
                    ...
                    await msg.answer(
                        ...
                        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                            [InlineKeyboardButton(text="✅ Я оплатил", callback_data=f"check_crypto_{amount}", icon_custom_emoji_id=PREMIUM_IDS["check"])]
                        ]),
                        parse_mode=ParseMode.HTML
                    )
                else:
...
async def process_withdraw(msg: Message, state: FSMContext):
    ...
    try:
        ...
        for admin in ADMIN_IDS:
            keyboard = InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="✅ Одобрить", callback_data=f"approve_{msg.from_user.id}_{amount}", icon_custom_emoji_id=PREMIUM_IDS["check"]),
                 InlineKeyboardButton(text="❌ Отклонить", callback_data=f"reject_{msg.from_user.id}_{amount}", icon_custom_emoji_id=PREMIUM_IDS["cross"])]
            ])
...
async def create_duel(msg: Message):
    ...
    try:
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="⚔️ Принять дуэль", callback_data=f"accept_duel_{bet}_{msg.from_user.id}", icon_custom_emoji_id=PREMIUM_IDS["game"])]
        ])
