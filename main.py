...
# ========== ФУНКЦИЯ ДЛЯ ПРЕМИУМ ОФОРМЛЕНИЯ ==========
def premium(text: str) -> str:
    """Только для оформления: /start, рефералка, перевод, меню"""
    replacements = {
...
def main_kb():
    return ReplyKeyboardMarkup(
        keyboard=[[
            KeyboardButton(text="Игры в боте", icon_custom_emoji_id=PREMIUM["game"]),
            KeyboardButton(text="Профиль", icon_custom_emoji_id=PREMIUM["profile"])
        ]],
...
def games_kb():
    ...
    b.row(InlineKeyboardButton(text="Чат", url=CHAT_LINK, icon_custom_emoji_id=PREMIUM["chat"]))
    b.row(InlineKeyboardButton(text="Назад", callback_data="back_main", icon_custom_emoji_id=PREMIUM["back"]))
    return b.as_markup()

def profile_kb():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Пополнить", callback_data="deposit", icon_custom_emoji_id=PREMIUM["deposit"]),
         InlineKeyboardButton(text="Вывести", callback_data="withdraw", icon_custom_emoji_id=PREMIUM["withdraw"])],
        [InlineKeyboardButton(text="Промокоды", callback_data="promo", icon_custom_emoji_id=PREMIUM["gift"]),
         InlineKeyboardButton(text="Рефералы", callback_data="ref", icon_custom_emoji_id=PREMIUM["ref"])],
        [InlineKeyboardButton(text="Перевод", callback_data="transfer", icon_custom_emoji_id=PREMIUM["send"]),
         InlineKeyboardButton(text="Назад", callback_data="back_main", icon_custom_emoji_id=PREMIUM["back"])]
    ])

def deposit_kb():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="CryptoBot", callback_data="dep_crypto", icon_custom_emoji_id=PREMIUM["crypto"])],
        [InlineKeyboardButton(text="Stars", callback_data="dep_stars", icon_custom_emoji_id=PREMIUM["star"])],
        [InlineKeyboardButton(text="Назад", callback_data="back_profile", icon_custom_emoji_id=PREMIUM["back"])]
    ])

def stars_amount_kb():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="50 Stars ($0.55)", callback_data="stars_50", icon_custom_emoji_id=PREMIUM["star"])],
        [InlineKeyboardButton(text="100 Stars ($1.10)", callback_data="stars_100", icon_custom_emoji_id=PREMIUM["star"])],
        [InlineKeyboardButton(text="250 Stars ($2.75)", callback_data="stars_250", icon_custom_emoji_id=PREMIUM["star"])],
        [InlineKeyboardButton(text="500 Stars ($5.50)", callback_data="stars_500", icon_custom_emoji_id=PREMIUM["star"])],
        [InlineKeyboardButton(text="1000 Stars ($11.00)", callback_data="stars_1000", icon_custom_emoji_id=PREMIUM["star"])],
        [InlineKeyboardButton(text="Назад", callback_data="back_deposit", icon_custom_emoji_id=PREMIUM["back"])]
    ])
...
def cancel_kb(callback_data: str = "cancel"):
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Отмена", callback_data=callback_data, icon_custom_emoji_id=PREMIUM["cross"])]
    ])
...
def init_db():
    ...
    cur.execute('''CREATE TABLE IF NOT EXISTS check_used (
        check_id TEXT,
        uid INTEGER,
        activated_at TEXT
    )''')
...
async def start(msg: Message):
    ...
    if await check_subscription(msg.from_user.id):
        txt = premium("""💰 Деньги есть всегда!

⚡️ Бот с играми и дуэлями
⚡️ Быстрые ставки, моментальные выводы
💵 Доход с рефералов
<i>От 💵100 в день — реально</i>""")
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text="Проверить", callback_data="check_sub", icon_custom_emoji_id=PREMIUM["check"])]
        ])
        await msg.answer(
            "⚠️ Для использования бота необходимо подписаться на наш канал!\n\n👇 Нажмите на кнопку ниже, подпишитесь и нажмите «Проверить»",
...
async def help_cmd(msg: Message):
    await msg.answer(
        ...
        "🎯 Дартс — /target {сумма}\n"
        "⚔️ Дуэль — /cub {ставка}\n\n"
        "📤 Перевод — /send {сумма} @username\n\n"
        "⭐️ Пополнение — в личных сообщениях с ботом\n"
        "💸 Вывод — в личных сообщениях с ботом\n\n"
        "Подпишись на канал для игры!",
        parse_mode=ParseMode.HTML
    )
...
# === ИГРЫ В ЛИЧКЕ ===
@dp.message(F.text == "Игры в боте", F.chat.type == "private")
async def games_menu(msg: Message):
    if not await check_subscription(msg.from_user.id):
        return
    u = get_user(msg.from_user.id)
    await msg.answer(f"🎮 Выберите игру!\n\n💰 Баланс: ${u['balance']:.2f}", reply_markup=games_kb())
...
async def back_deposit(c: CallbackQuery):
    await c.message.edit_text("💳 Пополнение средств", reply_markup=deposit_kb())
    await c.answer()
...
async def ref_prog(c: CallbackQuery):
    if not await check_subscription(c.from_user.id):
        ...
        return
    u = get_user(c.from_user.id)
    txt = premium(f"""👥 Реферальная программа
...
async def promo_menu(c: CallbackQuery, state: FSMContext):
    if not await check_subscription(c.from_user.id):
        await c.answer("❌ Подпишитесь на канал!", show_alert=True)
        return
    await c.message.edit_text("🎁 Введи промокод:\n\nПросто напиши код в чат — и бонус зачислится автоматически.", reply_markup=back_kb())
...
async def promo_use(msg: Message, state: FSMContext):
    ...
    if p[3] >= p[2] and p[2] > 0:
        ...
        await state.clear()
        return
    if p[6] and datetime.now() > datetime.strptime(p[6], "%Y-%m-%d"):
        await msg.answer("⭐️ Срок действия промокода истек!", reply_markup=back_kb())
        await state.clear()
        return
    user = get_user(msg.from_user.id)
    if p[5] == 1 and user.get("is_premium", 0) == 0:
        await msg.answer("⭐️ Этот промокод только для премиум пользователей!", reply_markup=back_kb())
        await state.clear()
        return
    if p[4] > 0 and user.get("deposit", 0) < p[4]:
        await msg.answer(f"⭐️ Для активации нужен депозит от ${p[4]}!", reply_markup=back_kb())
        await state.clear()
        return
    if p[5] > 0 and user.get("wagered", 0) < p[5]:
        await msg.answer(f"⭐️ Для активации нужен оборот от ${p[5]}!", reply_markup=back_kb())
        await state.clear()
        return
    
    upd_bal(msg.from_user.id, p[1])
    cur.execute("INSERT INTO promo_used VALUES (?, ?, ?)", (code, msg.from_user.id, datetime.now()))
    cur.execute("UPDATE promos SET activated = activated + 1 WHERE code = ?", (code,))
    conn.commit()
    conn.close()
    await msg.answer(f"🎉 Промокод активирован! +${p[1]:.2f} на баланс!")
    await state.clear()
...
async def transfer_amount(msg: Message, state: FSMContext):
    try:
        ...
        await msg.answer(
            premium("📤 Введите username или ID получателя:\n\nПримеры:\n• @username\n• 123456789"),
            reply_markup=cancel_kb("cancel_transfer")
        )
...
async def transfer_target(msg: Message, state: FSMContext):
    ...
    if msg.text.startswith("@"):
        ...
        try:
            ...
            upd_username(target_id, username)
        except:
            await msg.answer(f"❌ Пользователь @{username} не найден в Telegram!")
            return
    else:
        try:
            target_id = int(msg.text)
        except:
            await msg.answer("❌ Введите корректный username или ID!")
            return
    
    if target_id == msg.from_user.id:
        await msg.answer("❌ Нельзя перевести деньги самому себе!")
        return
    ...
    await msg.answer(
        ...
        f"📥 Получатель: {target_id} (@{target_username if target_username else ''})\n"
        f"💰 Сумма: ${amount:.2f}\n\n"
...
async def game_start(c: CallbackQuery, state: FSMContext):
    ...
    await c.message.answer(f"✏️ Введите сумму ставки (мин. ${MIN_BET:.2f})", reply_markup=cancel_kb("cancel_bet"))
...
async def process_bet(msg: Message, state: FSMContext):
    ...
    if game == "dice":
        ...
        if value >= 4:
            ...
            await msg.answer(f"🎲 Выпало: {value}\n✅ Выигрыш: ${win:.2f} (x{mult})!")
        else:
    ...
    await msg.answer(f"🎮 Выберите игру!\n\n💰 Баланс: ${u2['balance']:.2f}", reply_markup=games_kb())
...
async def play_tower(msg: Message, state: FSMContext, level: int, mult: float, bet: float):
    ...
    b.row(InlineKeyboardButton(text="Отмена", callback_data="cancel_tower", icon_custom_emoji_id=PREMIUM["cross"]))
...
async def tower_choice(c: CallbackQuery, state: FSMContext):
    ...
    if c.data == "tower_cashout":
        ...
        if level == 0:
            await c.answer("⚠️ Нужно открыть хотя бы одну клетку!", show_alert=True)
            return
        win = bet * mult
        ...
        await state.clear()
        u = get_user(c.from_user.id)
        await c.message.answer(f"🎮 Выберите игру!\n\n💰 Баланс: ${u['balance']:.2f}", reply_markup=games_kb())
        await c.answer()
        return
                ...
                if result.get("ok"):
                    ...
                    await msg.answer(
                        ...
                        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                            [InlineKeyboardButton(text="✅ Я оплатил", callback_data=f"check_crypto_{amount}", icon_custom_emoji_id=PREMIUM["check"])]
                        ]),
                        parse_mode=ParseMode.HTML
                    )
                else:
...
async def dep_stars(c: CallbackQuery):
    if not await check_subscription(c.from_user.id):
        await c.answer("❌ Подпишитесь на канал!", show_alert=True)
        return
    await c.message.edit_text("⭐️ Пополнение через Telegram Stars\n\nВыберите сумму:", reply_markup=stars_amount_kb())
    await c.answer()
...
async def process_withdraw(msg: Message, state: FSMContext):
    ...
    try:
        ...
        for admin in ADMIN_IDS:
            keyboard = InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="✅ Одобрить", callback_data=f"approve_{msg.from_user.id}_{amount}", icon_custom_emoji_id=PREMIUM["check"]),
                 InlineKeyboardButton(text="❌ Отклонить", callback_data=f"reject_{msg.from_user.id}_{amount}", icon_custom_emoji_id=PREMIUM["cross"])]
            ])
...
async def admin_create_promo(msg: Message):
    ...
    try:
        ...
        conn = sqlite3.connect('data.db')
        cur = conn.cursor()
        cur.execute("INSERT INTO promos (code, reward, max_act) VALUES (?, ?, ?)",
                   (code, reward, max_act))
        conn.commit()
        conn.close()
...
async def send_money(msg: Message):
    ...
    parts = msg.text.split()
    if len(parts) < 3:
        await msg.reply("❌ Использование: /send {сумма} {username или id}\n\nПример:\n/send 0.5 @username\n/send 1.5 123456789")
        return
    
    try:
        amount = float(parts[1].replace(",", "."))
        if amount < MIN_TRANSFER:
            await msg.reply(f"❌ Минимальная сумма перевода ${MIN_TRANSFER:.2f}!")
            return
        ...
        if target.startswith("@"):
            ...
            try:
                ...
                # Если не нашли через API, пробуем найти в БД
                conn = sqlite3.connect('data.db')
                cur = conn.cursor()
                ...
                if row:
                    ...
                    await msg.reply(f"❌ Пользователь @{username} не найден! Убедитесь, что username правильный и пользователь запускал бота хотя бы раз.")
                    return
        else:
            try:
                target_id = int(target)
            except:
                await msg.reply("❌ Неверный формат получателя!")
                return
        
        if target_id == msg.from_user.id:
            await msg.reply("❌ Нельзя перевести деньги самому себе!")
            return
        
        # Проверяем, существует ли пользователь в нашей базе (создаём если нет)
        ...
        await msg.reply(
            f"✅ Перевод выполнен!\n\n"
            f"📤 Отправитель: {msg.from_user.id}\n"
            f"📥 Получатель: {target_id} (@{username if target.startswith('@') else ''})\n"
            f"💰 Сумма: ${amount:.2f}\n\n"
            f"💰 Ваш баланс: ${get_user(msg.from_user.id)['balance']:.2f}"
        )
        ...
        await msg.reply("❌ Ошибка! Использование: /send {сумма} {username или id}\n\nПример:\n/send 0.5 @username\n/send 1.5 123456789")
...
async def create_duel(msg: Message):
    ...
    try:
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="⚔️ Принять дуэль", callback_data=f"accept_duel_{bet}_{msg.from_user.id}", icon_custom_emoji_id=PREMIUM["game"])]
        ])
...
async def chat_dice(msg: Message):
    ...
    try:
        ...
        if value >= 4:
            ...
            upd_bal(msg.from_user.id, win)
            upd_wagered(msg.from_user.id, bet)
            await msg.reply(f"🎲 Выпало: {value}\n✅ Выигрыш: ${win:.2f} (x{mult})!")
        else:
            upd_lost(msg.from_user.id, bet)
            await msg.reply(f"🎲 Выпало: {value}\n❌ Проигрыш: ${bet:.2f}!")
    except:
...
async def create_check(msg: Message):
    if msg.from_user.id not in ADMIN_IDS:
        return
    ...
    try:
        ...
        max_act = int(parts[2]) if len(parts) > 2 else 0
        
        check_id = generate_check_id()
        
        conn = sqlite3.connect('data.db')
        cur = conn.cursor()
        cur.execute("INSERT INTO checks (check_id, amount, max_act, created_by, created_at, expires) VALUES (?, ?, ?, ?, ?, ?)",
                   (check_id, amount, max_act, msg.from_user.id, datetime.now(), (datetime.now() + timedelta(days=30)).strftime("%Y-%m-%d")))
        conn.commit()
        conn.close()
        
        await msg.answer(f"✅ Чек создан!\n💰 Сумма: ${amount}\n🔗 Ссылка: https://t.me/{BOT_USERNAME}?start=check_{check_id}\n🔢 Лимит: {max_act if max_act > 0 else '∞'}")
    except:
        await msg.answer("❌ /create_check {сумма} {лимит}")

@dp.message(Command("start"), F.text.contains("check_"))
async def activate_check(msg: Message):
    check_id = msg.text.split("check_")[1].strip()
    ...
    if check[3] >= check[2] and check[2] > 0:
        await msg.answer("❌ Чек уже использован максимальное количество раз!")
        return
    
    if check[7] and datetime.now() > datetime.strptime(check[7], "%Y-%m-%d"):
        await msg.answer("❌ Срок действия чека истек!")
        return
    
    cur.execute("SELECT 1 FROM check_used WHERE check_id = ? AND uid = ?", (check_id, msg.from_user.id))
    if cur.fetchone():
        await msg.answer("❌ Вы уже активировали этот чек!")
        return
    
    upd_bal(msg.from_user.id, check[1])
    cur.execute("INSERT INTO check_used VALUES (?, ?, ?)", (check_id, msg.from_user.id, datetime.now()))
    cur.execute("UPDATE checks SET activated = activated + 1 WHERE check_id = ?", (check_id,))
    conn.commit()
    conn.close()
    
    await msg.answer(f"🎉 Чек активирован! +${check[1]:.2f} на баланс!")
