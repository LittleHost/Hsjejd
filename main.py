...
PREMIUM = {
    ...
    "pen":   "5258179403652801593",
    "back":  "5258236805890710909",
    "tower": "5188481279963715781",
    "chat": "5325932336653545675",
    "send": "5931250188239770853",
    "diamond": "5323546959061989444",
    "pop": "5415796104891483565",
}

# Функция для премиум эмодзи в ТЕКСТАХ
def p(text: str) -> str:
    """Заменяет обычные эмодзи на премиум через tg-emoji теги"""
    replacements = {
        ...
        "⚠️": f'<tg-emoji emoji-id="{PREMIUM["alert"]}">⚠️</tg-emoji>',
        "✏️": f'<tg-emoji emoji-id="{PREMIUM["pen"]}">✏️</tg-emoji>',
        "🔙": f'<tg-emoji emoji-id="{PREMIUM["back"]}">🔙</tg-emoji>',
        "🗼": f'<tg-emoji emoji-id="{PREMIUM["tower"]}">🗼</tg-emoji>',
        "💬": f'<tg-emoji emoji-id="{PREMIUM["chat"]}">💬</tg-emoji>',
...
<i>От 💵100 в день — реально</i>""")
        ...
        await msg.answer(
            p("⚠️ <b>Для использования бота необходимо подписаться на наш канал!</b>\n\n👇 Нажмите на кнопку ниже, подпишитесь и нажмите «Проверить подписку»"),
...
async def check_sub_callback(c: CallbackQuery):
    if await check_subscription(c.from_user.id):
        ...
        await c.answer(p("✅ Подписка подтверждена! Добро пожаловать!"), show_alert=True)
    else:
        await c.answer(p("❌ Вы не подписаны на канал! Подпишитесь и нажмите снова."), show_alert=True)
...
async def balance_cmd(msg: Message):
    if not await check_subscription(msg.from_user.id):
        await msg.reply(p("⚠️ Подпишитесь на канал!"))
        return
    u = get_user(msg.from_user.id)
...
async def play_tower(msg: Message, state: FSMContext, level: int, mult: float, bet: float):
    ...
    b.row(InlineKeyboardButton(text=p("❌ Отмена"), callback_data="cancel_tower", icon_custom_emoji_id=PREMIUM["cross"]))
                ...
                if result.get("ok"):
                    ...
                    await msg.answer(
                        ...
                        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                            [InlineKeyboardButton(text=p("✅ Я оплатил"), callback_data=f"check_crypto_{amount}", icon_custom_emoji_id=PREMIUM["check"])]
                        ]),
                        parse_mode=ParseMode.HTML
                    )
                else:
...
async def process_withdraw(msg: Message, state: FSMContext):
    ...
    try:
        ...
        for admin in ADMIN_IDS:
            keyboard = InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text=p("✅ Одобрить"), callback_data=f"approve_{msg.from_user.id}_{amount}", icon_custom_emoji_id=PREMIUM["check"]),
                 InlineKeyboardButton(text=p("❌ Отклонить"), callback_data=f"reject_{msg.from_user.id}_{amount}", icon_custom_emoji_id=PREMIUM["cross"])]
            ])
...
async def send_money(msg: Message):
    if not await check_subscription(msg.from_user.id):
        await msg.answer(p("⚠️ Подпишитесь на канал!"))
        return
    
    parts = msg.text.split()
    if len(parts) < 3:
        await msg.answer(p("❌ Использование: /send {сумма} {username или id}\n\nПример:\n/send 0.5 @username\n/send 1.5 123456789"))
        return
...
async def create_duel(msg: Message):
    ...
    try:
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=p("⚔️ Принять дуэль"), callback_data=f"accept_duel_{bet}_{msg.from_user.id}", icon_custom_emoji_id=PREMIUM["game"])]
        ])
