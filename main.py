...
# Функция для премиум эмодзи (только для нужных мест)
def pm(text: str) -> str:
    """Премиум эмодзи для меню, профиля, рефералки, промокодов, перевода"""
    replacements = {
        ...
        "✅": f'<tg-emoji emoji-id="{PREMIUM["check"]}">✅</tg-emoji>',
        "❌": f'<tg-emoji emoji-id="{PREMIUM["cross"]}">❌</tg-emoji>',
        "🗼": f'<tg-emoji emoji-id="{PREMIUM["tower"]}">🗼</tg-emoji>',
    }
...
def main_kb():
    return ReplyKeyboardMarkup(
        keyboard=[[
            KeyboardButton(text=pm("Игры в боте"), icon_custom_emoji_id=PREMIUM["game"]),
            KeyboardButton(text=pm("Профиль"), icon_custom_emoji_id=PREMIUM["profile"])
        ]],
...
def games_kb():
    ...
    b.row(InlineKeyboardButton(text=pm("Чат"), url=CHAT_LINK, icon_custom_emoji_id=PREMIUM["chat"]))
    b.row(InlineKeyboardButton(text=pm("Назад"), callback_data="back_main", icon_custom_emoji_id=PREMIUM["back"]))
    return b.as_markup()

def profile_kb():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=pm("Пополнить"), callback_data="deposit", icon_custom_emoji_id=PREMIUM["deposit"]),
         InlineKeyboardButton(text=pm("Вывести"), callback_data="withdraw", icon_custom_emoji_id=PREMIUM["withdraw"])],
        [InlineKeyboardButton(text=pm("Промокоды"), callback_data="promo", icon_custom_emoji_id=PREMIUM["gift"]),
         InlineKeyboardButton(text=pm("Рефералы"), callback_data="ref", icon_custom_emoji_id=PREMIUM["ref"])],
        [InlineKeyboardButton(text=pm("Перевод"), callback_data="transfer", icon_custom_emoji_id=PREMIUM["send"]),
         InlineKeyboardButton(text=pm("Назад"), callback_data="back_main", icon_custom_emoji_id=PREMIUM["back"])]
    ])

def deposit_kb():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=pm("CryptoBot"), callback_data="dep_crypto", icon_custom_emoji_id=PREMIUM["crypto"])],
        [InlineKeyboardButton(text=pm("Stars"), callback_data="dep_stars", icon_custom_emoji_id=PREMIUM["star"])],
        [InlineKeyboardButton(text=pm("Назад"), callback_data="back_profile", icon_custom_emoji_id=PREMIUM["back"])]
    ])

def stars_amount_kb():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=pm("50 Stars ($0.55)"), callback_data="stars_50", icon_custom_emoji_id=PREMIUM["star"])],
        [InlineKeyboardButton(text=pm("100 Stars ($1.10)"), callback_data="stars_100", icon_custom_emoji_id=PREMIUM["star"])],
        [InlineKeyboardButton(text=pm("250 Stars ($2.75)"), callback_data="stars_250", icon_custom_emoji_id=PREMIUM["star"])],
        [InlineKeyboardButton(text=pm("500 Stars ($5.50)"), callback_data="stars_500", icon_custom_emoji_id=PREMIUM["star"])],
        [InlineKeyboardButton(text=pm("1000 Stars ($11.00)"), callback_data="stars_1000", icon_custom_emoji_id=PREMIUM["star"])],
        [InlineKeyboardButton(text=pm("Назад"), callback_data="back_deposit", icon_custom_emoji_id=PREMIUM["back"])]
    ])
...
def cancel_kb(callback_data: str = "cancel"):
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=pm("Отмена"), callback_data=callback_data, icon_custom_emoji_id=PREMIUM["cross"])]
    ])
...
async def back_deposit(c: CallbackQuery):
    await c.message.edit_text(pm("💳 Пополнение средств"), reply_markup=deposit_kb(), parse_mode=ParseMode.HTML)
    await c.answer()
...
async def promo_use(msg: Message, state: FSMContext):
    ...
    p = cur.fetchone()
    if not p:
        await msg.answer(pm("⭐️ Промокод не найден!"), reply_markup=back_kb())
        await state.clear()
        return
    cur.execute("SELECT 1 FROM promo_used WHERE code = ? AND uid = ?", (code, msg.from_user.id))
    if cur.fetchone():
        await msg.answer(pm("⭐️ Вы уже активировали этот промокод!"), reply_markup=back_kb())
        await state.clear()
        return
    if p[3] >= p[2] and p[2] > 0:
        await msg.answer(pm("⭐️ Промокод достиг лимита!"), reply_markup=back_kb())
        await state.clear()
        return
    ...
    conn.commit()
    conn.close()
    await msg.answer(pm(f"🎉 Промокод активирован! +${p[1]:.2f} на баланс!"))
    await state.clear()
...
async def play_tower(msg: Message, state: FSMContext, level: int, mult: float, bet: float):
    ...
    b.row(InlineKeyboardButton(text=pm("❌ Отмена"), callback_data="cancel_tower", icon_custom_emoji_id=PREMIUM["cross"]))
...
async def process_withdraw(msg: Message, state: FSMContext):
    ...
    try:
        ...
        for admin in ADMIN_IDS:
            keyboard = InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text=pm("✅ Одобрить"), callback_data=f"approve_{msg.from_user.id}_{amount}", icon_custom_emoji_id=PREMIUM["check"]),
                 InlineKeyboardButton(text=pm("❌ Отклонить"), callback_data=f"reject_{msg.from_user.id}_{amount}", icon_custom_emoji_id=PREMIUM["cross"])]
            ])
