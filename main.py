                ...
                if result.get("ok"):
                    ...
                    await msg.answer(
                        ...
                        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                            [InlineKeyboardButton(text="✅ Я оплатил", callback_data=f"check_crypto_{amount}", icon_custom_emoji_id=PREMIUM_BUTTONS["check"])]
                        ]),
                        parse_mode=ParseMode.HTML
                    )
                else:
...
async def process_withdraw(msg: Message, state: FSMContext):
    ...
    try:
        ...
        for admin in ADMIN_IDS:
            keyboard = InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="✅ Одобрить", callback_data=f"approve_{msg.from_user.id}_{amount}", icon_custom_emoji_id=PREMIUM_BUTTONS["check"]),
                 InlineKeyboardButton(text="❌ Отклонить", callback_data=f"reject_{msg.from_user.id}_{amount}", icon_custom_emoji_id=PREMIUM_BUTTONS["cross"])]
            ])
