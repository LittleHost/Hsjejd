...
def main_kb():
    return ReplyKeyboardMarkup(
        keyboard=[[
            KeyboardButton(text=em("🎮 Игры в боте")),
            KeyboardButton(text=em("👤 Профиль"))
        ]],
...
<i>От 💵100 в день — реально</i>""")
        ...
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text=em("📢 Подписаться"), url=REQUIRED_CHANNEL_LINK, icon_custom_emoji_id=E["chat"])],
...
# === ПЕРЕВОД МЕЖДУ ИГРОКАМИ (ДЛЯ ВСЕХ) ===
...
# === КОМАНДА /send ДЛЯ ВСЕХ ИГРОКОВ ===
@dp.message(Command("send"), F.chat.type == "private")
async def send_money(msg: Message):
    if not await check_subscription(msg.from_user.id):
        await msg.answer(em("⚠️ Подпишитесь на канал @SecondNewsRU!"))
        return
...
async def accept_duel(c: CallbackQuery):
    ...
    if not await check_subscription(c.from_user.id):
        await c.answer(em("❌ Подпишитесь на канал @SecondNewsRU!"), show_alert=True)
        return
